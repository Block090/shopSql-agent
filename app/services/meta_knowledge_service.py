...
class MetaKnowledgeService:
    ...
    async def _save_column_info_to_qdrant(self, column_infos: list[ColumnInfo]):
        ...
        await self.column_qdrant_repository.ensure_collection(reset=True)
            ...
            points.append(
                {
                    ...
                    "payload": self._versioned_payload(column_info),
                }
            )
    ...
    async def _save_value_info_to_es(
        ...
        await self.value_es_repository.ensure_index(reset=True)
        ...
        await self.value_es_repository.index(
            value_infos, index_version=self.index_version
        )
    ...
    async def _save_metrics_to_qdrant(self, metric_infos: list[MetricInfo]):
        ...
        await self.metric_qdrant_repository.ensure_collection(reset=True)
            ...
            points.append(
                {
                    ...
                    "payload": self._versioned_payload(metric_info),
                }
            )
    ...
    async def build(self, config_path: Path):
        ...
        self.index_version = meta_config.version or datetime.now().strftime(
            "%Y%m%d%H%M%S"
        )
        ...
        self._write_build_manifest(meta_config)

    def _versioned_payload(self, entity) -> dict:
        payload = asdict(entity)
        payload["_index_version"] = self.index_version
        return payload

    def _write_build_manifest(self, meta_config: MetaConfig) -> None:
        manifest_path = Path("logs/meta_knowledge_manifest.json")
        manifest_path.parent.mkdir(parents=True, exist_ok=True)
        manifest = {
            "index_version": self.index_version,
            "built_at": datetime.now().isoformat(timespec="seconds"),
            "table_count": len(meta_config.tables or []),
            "metric_count": len(meta_config.metrics or []),
            "targets": [
                "meta_mysql",
                "qdrant_columns",
                "qdrant_metrics",
