...
class MetaMySQLRepository:
    ...
    async def get_column_infos_by_ids(self, ids: list[str]) -> list[ColumnInfo]:
        ...
        async with self._session_lock:
            for column_id in dict.fromkeys(ids):
                ...
                if model:
                    entities.append(ColumnInfoMapper.to_entity(model))
        return entities

    async def get_metric_infos_by_ids(self, ids: list[str]) -> list[MetricInfo]:
        """按业务词典命中的指标 id 批量补齐确定性召回结果。"""

        entities = []
        async with self._session_lock:
            for metric_id in dict.fromkeys(ids):
                model = await self.session.get(MetricInfoMySQL, metric_id)
                if model:
                    entities.append(MetricInfoMapper.to_entity(model))
        return entities
    ...
    async def get_table_info_by_id(self, id: str) -> TableInfo | None:
        ...
        async with self._session_lock:
            table_info: TableInfoMySQL | None = await self.session.get(
                TableInfoMySQL, id
            )
        if table_info:
