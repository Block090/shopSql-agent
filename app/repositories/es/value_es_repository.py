...
class ValueESRepository:
    ...
    async def ensure_index(self, reset: bool = False):
        """确保字段取值索引已经创建好"""
        if reset and await self.client.indices.exists(index=self.index_name):
    ...
    async def index(
        ...
        for i in range(0, len(value_infos), batch_size):
            ...
            for value_info in batch_value_infos:
                ...
                source = asdict(value_info)
                source["_index_version"] = index_version
                batch_operations.append(source)
