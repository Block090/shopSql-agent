...
from app.core.business_lexicon import get_required_column_ids
from app.core.log import logger
from app.core.query_case_memory import compress_similar_cases
from app.core.rag_reranker import build_retrieval_query
...
async def merge_retrieved_info(
    ...
    try:
        ...
        await complete_required_column_context(
            query=state["query"],
            column_info_map=retrieved_column_infos_map,
        ...
        similar_query_cases = await _recall_similar_query_cases(
            query=state["query"],
        ...
        return {
            ...
            "similar_query_cases": similar_query_cases,
        }
...
async def complete_required_column_context(
    query: str,
    column_info_map: dict[str, ColumnInfo],
    meta_mysql_repository,
) -> None:
    """Add deterministic business columns that retrieval or reranking dropped."""
    ...
    if not missing_column_ids:
        return

    required_columns = await meta_mysql_repository.get_column_infos_by_ids(
        missing_column_ids
    )
...
async def _recall_similar_query_cases(
    query: str,
    semantic_slots: dict,
    runtime: Runtime[DataAgentContext],
) -> list[dict]:
    """召回相似历史成功查询案例，失败时返回空列表。"""

    query_case_repository = runtime.context.get("query_case_repository")
    if not query_case_repository:
        return []

    try:
        embedding_client = runtime.context["embedding_client"]
        retrieval_query = build_retrieval_query(query, semantic_slots)
        embedding = await embedding_client.aembed_query(retrieval_query)
        cases = await query_case_repository.search_query_cases(embedding, limit=3)
        return compress_similar_cases(cases)
    except Exception as exc:
        logger.warning(f"召回 Query Case 失败，已跳过增强记忆：{exc}")
        return []
