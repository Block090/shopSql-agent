...
from app.core.business_lexicon import expand_business_terms
from app.core.log import logger
from app.core.value_normalizer import expand_value_terms
...
async def recall_value(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    ...
    try:
        ...
        result = await run_with_timeout(
            ...
            timeout_seconds=8,
            default=[],
            operation_name="字段取值 LLM 扩展",
        )

        # 通用关键词、LLM 扩展词和规则归一化别名一起检索 ES。
        # 例如“第一季度”会额外搜索 Q1，“华东地区”会额外搜索 华东。
        keywords = set(
            expand_value_terms(expand_business_terms(query, keywords + result))
        )

        # 用 ValueInfo.id 去重，避免多个关键词命中同一条字段值记录
        value_infos_map: dict[str, ValueInfo] = {}
        for keyword in keywords:
            current_value_infos: list[ValueInfo] = await run_with_timeout(
                value_es_repository.search(keyword),
                timeout_seconds=5,
                default=[],
                operation_name=f"字段取值 ES 检索({keyword})",
            )
